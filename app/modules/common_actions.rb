module CommonActions
  ...
  def self.clear_temp_objects(_params = {})
  ...
  def application_main_menu
    ...
    if user_signed_in? && !current_user.is_vendor? && !current_user.is_customer?
      ...
      if can? :view, CompanyInfo
        menus[:system][:sub_menu].push(path: company_infos_path, name: 'Corporate Info')
      end
    end
    menus
  end
  ...
  def self.get_new_identifier(model, field, letter)
    ...
    if max_identifier.nil?
      letter + '00001'
    elsif (cur_identifier = max_identifier[1..5].to_i + 1) > 99_999
      letter + '00001'
    else
      letter + format('%05d', cur_identifier)
    end
  end
  ...
  def self.process_application_notifications(user_id)
    ...
    if User.current_user.present? && User.current_user.is_quality?
      ...
      vendor_organizations.each do |vendor_organization|
        next unless vendor_organization.present?
      ...
      specifications.each do |specification|
        next unless specification.present?
  ...
  def self.common_process_model(model, model_note, user)
    notification_set_status(model_note, model, user.id) if user.present?
  end
  ...
  def self.sales_report(so_id)
    ...
    @so_header.so_lines.each_with_index do |so_line, index|
      ...
      if i == 1
        ...
        if flag2 == 1
          content += '<article class="art-01 art-04 de"><div class="ff"><table border="0" width="640px" cellspacing="0" cellpadding="0"><tbody><tr align="center" class="hea art-002"><td style="font-weight: bold; padding:0 0 20px 0; font-size:16px;">CUST P/N - ALL P/N</td><td style="font-weight: bold; padding:0 0 20px 0; font-size:16px;">Revision</td><td style="font-weight: bold; padding:0 0 20px 0; font-size:16px;">Description</td>'
          content += '<td style="font-weight: bold; padding:0 0 20px 0; font-size:16px;">QTY</td><td style="font-weight: bold; padding:0 0 20px 0; font-size:16px;">Shipped</td></tr></tbody></table></div>'
          flag2 = 0
        else
          content += '<article class="art-01 art-04 de sal_tab2"><div class="ff"><table border="0" width="640px" cellspacing="0" cellpadding="0"><tbody><tr align="center" class="hea art-002"><td style="font-weight: bold; padding:0 0 20px 0; font-size:16px;">CUST P/N - ALL P/N</td><td style="font-weight: bold; padding:0 0 20px 0; font-size:16px;">Revision</td> <td style="font-weight: bold; padding:0 0 20px 0; font-size:16px;">Description</td>'
          content += '<td style="font-weight: bold; padding:0 0 20px 0; font-size:16px;">QTY</td><td style="font-weight: bold; padding:0 0 20px 0; font-size:16px;">Shipped</td></tr></tbody></table></div>'
        end
      end
      content += '<div class="fff"><table border="0" width="640px" cellspacing="0" cellpadding="0"><tbody><tr align="center" class="hea art-002"><td><table border="0" width="100%"><tbody><tr><td width="150" scope="row">' + item_part_no + '</td></tr> <tr><td width="150" scope="row">' + item_alt_name + '</td></tr> </tbody></table></td><td >' + item_revision.to_s + '</td><td><table border="0" width="100%"><tbody><tr> <td width="150" scope="row">' + item_description
      content += '</td></tr>   </tbody></table></td><td >' + so_line.so_line_quantity.to_s + '</td><td>' + so_line.so_line_shipped.to_s + '</td></tr><tr><td class="ww-01" style="color: #800000;">' + so_line_notes + ' &nbsp </td></tr></tbody></table></div>'
 ...
 <article>
    ...
    <div class="ms_image-2">
      ...
      content += if po_type_name == 'Transfer'
                   '<td>' + po_line_lot + '</td>
