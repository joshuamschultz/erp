class Organization < ActiveRecord::Base
	...
	def redirect_path
  	...
  	def po_items
		...
		po_items= PoLine.joins(:po_header).where("po_headers.organization_id = ?", self.id).order("po_headers.created_at DESC")
  	...
  	def so_items
		...
		so_items= SoLine.joins(:so_header).where("so_headers.organization_id = ?", self.id).order("so_headers.created_at DESC")
