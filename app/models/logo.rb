	...
	validates_presence_of :joint
  	...
  	def before_create_process
  		if self.jointable_type =="QualityDocument"
			...
			unless MasterType.find_by_type_value(self.joint_file_name)
				MasterType.create(type_name: self.joint_file_name, type_description: "", type_value: self.joint_file_name, type_category: "customer_quality_level", type_active: true)
			end
  		end
  	end
