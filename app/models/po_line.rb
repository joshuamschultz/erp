class PoLine < ActiveRecord::Base
  ...
  def generate_pdf
    ...
    po_cost = po_description = item_no = alt_item_no = qty = []
    ...
    item_no<< "<strong>"+(po_line.item.item_part_no).to_s+"</strong>"
    else 
    item_no<< "<strong>"+(po_line.item.item_part_no).to_s+"</strong>"
    alt_item_no<< "<strong>"+(po_line.item_alt_name.item_alt_identifier).to_s+"</strong>"
    end 
    if po_line.item_revision.present?
    po_description<< "<strong>"+(po_line.item_revision.item_description).to_s+ "</strong>"
    end 

    po_cost<< "<strong>"+(po_line.po_line_cost.to_f).to_s+"</strong>"
    end 
    ...
    temp = po_description.to_s
    temp = temp.gsub( /\[/, '')      
    temp = temp.gsub( /\]/, '')                          
    temp = temp.gsub( /\,/, '')                          
    po_description = temp.gsub( /\"/, '')  
    ...
    temp = temp.gsub( /\,/, '')                          
    item_no = temp.gsub( /\"/, '')  
    temp = alt_item_no.to_s
    temp = temp.gsub( /\[/, '')      
    temp = temp.gsub( /\]/, '')                          
    temp = temp.gsub( /\,/, '')                          
    alt_item_no = temp.gsub( /\"/, '')  
