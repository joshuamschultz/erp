class PoLine < ActiveRecord::Base
    ...
    po_line = shipment ? object.po_line : object
     ...
     if User.current_user.present? &&  !User.current_user.is_operations? && !User.current_user.is_clerical?
      ...
      unless shipment
        ...
        object[:po_identifier] = "<div style='background-color:#484848;height:30px;'><a href='/po_headers/#{po_line.po_header.id}' style='color: #8ec657;padding-left:10px;' >"+po_line.po_header.po_identifier+"</a> "
        ...
        object[:po_identifier] += "<a onclick='fill_po_items(#{po_line.po_header.id}); return false' class='pull-right btn btn-small btn-success po_#{po_line.po_header.id}' href='#'>Fill</a></div>"
  ...
  def generate_pdf
    ...
    html = CommonActions.purchase_report(self.po_header.id)+'<style>#blank_page{display: none;} .art-01.art-04.art-07{  min-height: 445px;} .art-01.art-04{  min-height: 445px;} article.art-05{min-height: 115px;} article.art-01{min-height: 70px;} article.art-02{margin-top: 61px;min-height: 60px;} @page{size:21cm 29.7cm;margin: 12mm 5mm 2mm 10mm;}</style>'
