class PoLine < ActiveRecord::Base
  ...
  qty = product1 = product2 = product_description  = product_cost  = product_notes = ''
  po_item_total = ''
              source = ''
         
  self.po_header.po_lines.each do |po_line| 
    qty+="<td width='70'>" +po_line.po_line_quantity.to_s+ "</td>"
    ...
    if po_line.item_revision.present?
      ...
      product_notes+="<td colspan='6' class='h-pad-04'>" +po_line.item_revision.item_notes.to_s+"</td>"

    end 

    product_cost+= "<td>"+(po_line.po_line_cost.to_f).to_s+"</td>"
    po_item_total= ""+(po_line.po_line_total.to_f).to_s+""

    source+="<tr valign='top' align='left' class='h-pad'><td width='70'>" +po_line.po_line_quantity.to_s+"<td width='150'><table width='100%'' border='0'><tbody>"+product1+""+product2+"</tbody></table></td></td><td width='150'>"+product_description+"</td><td width='150'>"+(po_line.po_line_cost.to_f).to_s+"</td><td width='150'>"+(po_line.po_line_total.to_f).to_s+"</td></tr><tr class='h-pad'><td colspan='6' class='h-pad-04'>" +po_line.item_revision.item_notes.to_s+"</td></tr>"
...
<img alt=Report_heading src=http://erp.chessgroupinc.com/#{@company_info.logo.joint.url(:original)} />                </div>
        ...
        <article class="art-01">
            ...
            <div class="ms_text-2">
                ...
                <div class="ms_text-6 ms-33">
                    <h2 class="ms_sub-heading">Chess Group Inc </h2> 
                    <strong>#{@company_info.company_address1}</strong>
                    <strong>#{@company_info.company_address2}</strong>
                </div>
            </div>
        ...
        <article class="art-02">
            ...
            <div class="ms_text-2">
                <div class="ms_text-6">
                    ...
                    <div class="ms_text-7 ms_text-9 "> <strong class="ms-1">P.O. Total :</strong>  



                    <strong class="ms-2"> #{self.po_header.po_total.to_f}</strong> 
                    </div>
                </div>
