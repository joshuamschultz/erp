class QualityLotMaterialsController < ApplicationController
  ...
  def index
    ...
    @material = @quality_lot.item_revision.material

    if @quality_lot.quality_lot_materials.empty?
        redirect_to new_quality_lot_material_path(quality_lot_id: @quality_lot.id)
    else
  ...
  def new
    ...
    @item = @item_revision.item
    @material = @quality_lot.item_revision.material
    @material_elements = @quality_lot.lot_material_elements
